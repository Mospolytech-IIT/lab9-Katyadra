...
@app.post("/users/new")
def create_user(
        username: str = Form(...), email: str = Form(...), password: str = Form(...), db:
...
def get_user(user_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("edit_user.html", {"request": request, "user":
        user})


@app.put("/users/edit/{user_id}")
def update_user(
        user_id: int, username: str = Form(...), email: str = Form(...), password: str =
...
def get_posts_page(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("posts.html", {"request": request, "posts":
        posts})
...
@app.post("/posts/new")
def create_post(
        title: str = Form(...), content: str = Form(...), user_id: int = Form(...), db:
        Session = Depends(get_db)
):
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
...
def get_post(post_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("edit_post.html", {"request": request, "post":
        post})


@app.put("/posts/edit/{post_id}")
def update_post(
        post_id: int, title: str = Form(...), content: str = Form(...), db: Session =
        Depends(get_db)
):
    post = db.query(Post).filter(Post.id == post_id).first()
    if not post:
